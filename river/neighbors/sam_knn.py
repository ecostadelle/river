        ...
        if (
            ...
            if len(self._ltm_labels) > self.max_ltm_size:
                ...
                if (
                    ...
                    stm_shortened = True
                    n_shifts = int(self.max_ltm_size -
                                   len(self._ltm_labels) + 1)
                    ...
                    self._stm_samples = np.delete(
                        self._stm_samples, shift_range, 0)
                    self._stm_labels = np.delete(
                        self._stm_labels, shift_range, 0)
                    self.stm_distances[
                        : len(self._stm_labels), : len(self._stm_labels)
                    ] = self.stm_distances[
        ...
        if len(self._stm_labels) > self.n_neighbors and samples_cl.shape[0] > 0:
            ...
            for i in loop_range:
                ...
                if len(correct_indices_stm) > 0:
                    ...
                    labels_cl = np.delete(
                        labels_cl, wrong_indices_ltm[del_indices], 0)
        ...
        if self.stm_aprox_adaption is not None:
            ...
            if new_window_size < old_window_size:
                ...
                if self.use_ltm:
                    ...
                    self._ltm_samples = np.vstack(
                        [self._ltm_samples, old_stm_samples])
                    self._ltm_labels = np.append(
                        self._ltm_labels, old_stm_labels)
                    self.size_check_fct()
    ...
    def _predict_by_stm(self, sample, label, distances_stm):  # noqa
        ...
        if curr_len > 0:
            predicted_label = self.get_labels_fct(
                distances_stm, self._stm_labels, min(
                    self.n_neighbors, curr_len)
            )[0]
        return predicted_label
...
class STMSizer:
    ...
    def get_new_stm_size(
        ...
        if aprox_adaption_strategy:
            ...
            raise Exception(
                f"Invalid adaption_strategy: {aprox_adaption_strategy}")
    ...
    def _get_interleaved_test_train_acc(
        ...
        for i in range(n_neighbours, len(labels)):
            ...
            predLabels.append(get_labels_fct(
                distances, labels[:i], n_neighbours)[0])
        return (
